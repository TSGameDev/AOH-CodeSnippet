...
namespace TSGameDev.Inventories.Actions
{
    ...
    public class ActionStore : MonoBehaviour, ISaveable
    {
        ...
        [SerializeField] private float maxThrowDis;
        ...
        public float GetThrowDis() => maxThrowDis;

        /// <summary>
        /// Getter for the thrownpoint transform
        /// </summary>
        /// <returns>Transform</returns>
        public Transform GetThrowPoint() => throwPoint;
        ...
        public bool Use(int index, GameObject user, Vector3 indicatorPos)
        {
            if (dockedItems.ContainsKey(index))
            {
                dockedItems[index].item.Use(user, indicatorPos, maxThrowDis);
        ...
        public void ActivateThrow(float _NumberPressed)
        {
            ActionItem _CurrentActiveItem = GetAction((int)_NumberPressed);
            if(_CurrentActiveItem != null)
            {
                _PlayerInputManager.SetThrowAction((int)_NumberPressed);
            }
        }
